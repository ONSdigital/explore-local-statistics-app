...
> Structural metadata for a specific *request* (not a whole indicator) is also available from the
...
| `hasGeo` | `all` | Restrict to indicators covering a GSS code, level, or type — **`all` is the "no filter" sentinel here; `any` is invalid**, the opposite of [the data endpoint](./data-endpoint.md#hasgeo) — see [gotchas.md](./gotchas.md#the-hasgeo-no-filter-sentinel-is-not-the-same-string-everywhere) |
...
[
  {
    ...
    "measure": "Percentage",
    "unit": "%",
    "geography": { "countries": ["E", "N", "S", "W"], "levels": ["ctry", "rgn", "cauth", "utla", "ltla"], "types": ["..."], "year": 2025, "initialLevel": "ltla" },
    "dimensions": {
...
```json
{
  ...
  "geography": {
    ...
    "types": ["E06", "E07", "E08", "E09", "E10", "E12", "E47", "E92", "K02", "K03", "K04", "N92", "S12", "S92", "W06", "W92"],
    "year": 2025,
    "initialLevel": "ltla"
  },
  "dimensions": {
...
```json
{
  "id": "period",
  "label": "Time period",
  "category": { "index": { "2004-01-01/P1Y": 0, "2005-01-01/P1Y": 1, "...": "...", "2023-01-01/P1Y": 19 } }
}
```
...
| `hasGeo` | `all` | Same semantics as [`/metadata/indicators`'s `hasGeo`](#get-apiv1metadataindicators) |
  ...
  "data": [
    {
      ...
      "slug": "population",
      "children": [
        { "label": "Total population", "slug": "population-count", "description": "Mid-year estimate of the total number of people", "index": 0 },
        { "label": "Population by age and sex", "slug": "population-by-age-and-sex", "description": "Percentage of the total population by sex and five-year age band", "index": 1 }
      ]
    }
  ]
}
...
```json
{
  "meta": { "count": 110, "total": 110 },
  "data": [
    { "label": "Total population", "slug": "population-count", "topic": "population", "subTopic": "population", "description": "Mid-year estimate of the total number of people" }
  ]
}
```
