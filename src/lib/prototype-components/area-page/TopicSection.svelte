			...
			<ul>
				{#if type !== 'table'}<li>
						<Icon type="chart" /><span
							><button class="btn-link" on:click={() => downloadPNG(el)}
								>Download {type} (PNG)</button
							></span
						>
					</li>{/if}
				{#if data}<li>
						<Icon type="download" /><span
							><button class="btn-link" on:click={() => downloadCSV(data)}
...
<style>
	.topic-section-container-1 {
		margin: 150px 0px 0px 0px;
	}

	.sub-topic-container-1 {
