import summaryStats from '$lib/data/json-stat-summary.json';
import readData from '$lib/data';
import { geoYearFilter, makeGeoFilter } from './geoFilters';
import { isValidAreaCode } from '$lib/util/validationHelpers';

const metadata = await readData('json-stat-metadata');

export function makeGeoExtentFilter(geo: string) {
	return isValidAreaCode(geo) ? (d: areaObject) => d.parents.includes(geo) : () => false;
}

export function makeGeoDatasetFilter(ds: jsonStatDataset) {
	return ds ? (d) => d.areacd in ds.dimension.areacd.category.index : () => false;
}

export function getLatestYear(ds: jsonStatDataset) {
	return ds ? ds.extension.geography.year : summaryStats.geoYears[summaryStats.geoYears.length - 1];
}

export default function makeAreaListFilter(geo, geoExtent, year, indicator) {
	if (geo === 'all' && year === 'all' && indicator === 'all') return null;
	const ds =
		indicator !== 'all'
			? metadata.link.item.find((ds: jsonStatDataset) => ds.extension.slug === indicator)
			: null;
	const yr = year === 'latest' ? getLatestYear(ds) : year;
	const yFilter = year === 'all' ? () => true : geoYearFilter;
	const gFilter = geo === 'all' ? () => true : makeGeoFilter([geo].flat());
	const eFilter = geoExtent === 'all' ? () => true : makeGeoExtentFilter(geoExtent);
	const iFilter = indicator === 'all' ? () => true : makeGeoDatasetFilter(ds);
	return (d: areaObject) => yFilter(d, yr) && eFilter(d) && gFilter(d) && iFilter(d);
}
